...
export default function Dashboard() {
  ...
  const [topWebsites, setTopWebsites] = useState<any[]>([])
  ...
  async function fetchTopWebsites() {
    ...
    if (res.ok) {
      ...
      if (data.success && Array.isArray(data.top_websites) && data.top_websites.length > 0) {
        setTopWebsites(
          data.top_websites.map((site: any) => ({
            site: site.url,
            category: "", // category is not returned by top_websites view
            time:
              site.total_time > 60
                ? `${Math.floor(site.total_time / 60)}m ${Math.round(site.total_time % 60)}s`
                : `${Math.round(site.total_time)}s`,
            visits: site.visits,
